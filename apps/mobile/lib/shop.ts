...
export function addToCart(token: string, productId: string, quantity = 1) {
  return apiFetch<Cart>('/shop/cart/items', {
    method: 'POST',
...
export function updateCartItem(token: string, itemId: string, quantity: number) {
  return apiFetch<Cart>(`/shop/cart/items/${itemId}`, {
    method: 'PATCH',
...
export function checkout(
  ...
  return apiFetch<ShopCheckoutResponse>('/shop/checkout', {
    method: 'POST',
...
export function uploadPrintDesign(
  ...
  return apiFetch<PrintDesignUpload>('/print/designs/upload', {
    method: 'POST',
    body: JSON.stringify(data),
  }, token);
}
...
export function createPrintOrder(
  ...
  return apiFetch<PrintCheckoutResponse>(`/print/listings/${listingId}/orders`, {
    method: 'POST',
    body: JSON.stringify(data),
  }, token);
}
