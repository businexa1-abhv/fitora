...
import { SPORT_LABELS, SportType, formatCurrency, type Venue } from '@fitora/shared';
...
function priceFrom(venue: Venue) {
  const n = Number(venue.priceFrom);
...
export function VenueCard({ venue, compact }: VenueCardProps) {
  ...
  const primarySportSlug = venue.sports[0]?.slug?.toUpperCase().replace(/-/g, '_') as
    SportType | undefined;
  const sport: SportType = primarySportSlug ?? SportType.OTHER;
  const sportColor = SPORT_COLORS[sport];
  const courtLabel = venue.courtCount === 1 ? '1 court' : `${venue.courtCount} courts`;
    ...
    >
      <Card padded={false} style={styles.card}>
        <View style={[styles.visual, { backgroundColor: `${sportColor}24` }]}>
          ...
          <View style={[styles.sportPill, { backgroundColor: colors.accent }]}>
            <Text style={styles.sportPillText}>
              {venue.sports[0] ? venue.sports[0].name : SPORT_LABELS[sport]}
            </Text>
          </View>
          <View style={[styles.courtPill, { backgroundColor: colors.card }]}>
            <Text style={[styles.courtPillText, { color: colors.foreground }]}>{courtLabel}</Text>
          </View>
        </View>
        <View style={styles.body}>
          ...
          {!compact && venue.amenities.length > 0 && (
          ...
          <View style={styles.footer}>
            <View>
              ...
              <Text style={[styles.price, { color: colors.foreground }]}>
                {formatCurrency(priceFrom(venue))}
